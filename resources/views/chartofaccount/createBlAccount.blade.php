	...
	<div class="max-w-4xl mx-auto py-10">
		...
		<div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
			...
			<div class="px-6 py-4 bg-gray-50 border-b">
				<h2 class="text-lg font-semibold text-gray-800">Create Balance Sheet Account</h2>
			</div>
			...
			<div class="px-6 py-6">
				...
				<form action="{{ route('BlAccountStore') }}" method="POST" class="space-y-6">
					@csrf
					...
					<div class="text-black">
						...
						<select name="account_group" required
							...
							@foreach(($groups ?? []) as $g)
								<option value="{{ $g }}" {{ old('account_group') == $g ? 'selected' : '' }}>{{ $g }}</option>
							@endforeach
						</select>
					...
					<div class="text-black">
						...
						<select name="cash_flow_category"
							...
							<option value="Operating activities" {{ old('cash_flow_category') == 'Operating activities' ? 'selected' : '' }}>Operating activities</option>
							<option value="Investing activities" {{ old('cash_flow_category') == 'Investing activities' ? 'selected' : '' }}>Investing activities</option>
							<option value="Financing activities" {{ old('cash_flow_category') == 'Financing activities' ? 'selected' : '' }}>Financing activities</option>
							<option value="Cash &amp; cash equivalents" {{ old('cash_flow_category') == 'Cash &amp; cash equivalents' ? 'selected' : '' }}>Cash &amp; cash equivalents</option>
						</select>
						@error('cash_flow_category')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
					</div>

					<div class="flex justify-end pt-4 border-t">
						<button type="submit" class="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-6 py-2.5 text-sm font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
							Create Account
						</button>
					</div>
				</form>
