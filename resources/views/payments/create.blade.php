<x-app-layout>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // --- FIXED SECTION START ---
            // We wrap the Blade directive in quotes to stop VS Code from seeing it as a "Decorator"
            const oldAllocData = '{!! json_encode(old("allocations", [])) !!}';
            try {
                const oldAlloc = JSON.parse(oldAllocData);
                if (Array.isArray(oldAlloc)) {
                    oldAlloc.forEach(a => {
                        if (a.invoice_id) addRow(a.invoice_id, a.amount);
                    });
                }
            } catch (e) {
                console.error("Failed to parse old allocations", e);
            }
            // --- FIXED SECTION END ---
        });
    </script>
