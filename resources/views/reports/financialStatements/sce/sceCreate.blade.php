<x-app-layout>
    <div class="max-w-4xl mx-auto text-black">
        <div class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h1 id="pageHeading" class="text-2xl font-semibold text-gray-800">Statement of Changes in Equity</h1>
                <p class="text-sm text-gray-500">Create a new Statement of Changes in Equity</p>
            </div>
        ...
        <div class="bg-white border border-gray-200 rounded-lg shadow-sm">
            <form action="{{ route('reports.financial.changes-in-equity.store') }}" method="POST" class="p-6 space-y-6">
                @csrf

                <div>
                    <label for="description" class="block text-sm font-medium text-gray-700">Description</label>
                    <input id="description" type="text" name="description" value="{{ old('description') }}" class="mt-1 w-full border-gray-300 rounded-md shadow-sm focus:ring-2 focus:ring-blue-200 focus:border-blue-500" />
                    @error('description')
                ...
                <div class="max-w-xs">
                    ...
                    <select id="accounting_method" name="accounting_method" class="mt-1 w-full border-gray-300 rounded-md shadow-sm focus:ring-2 focus:ring-blue-200 focus:border-blue-500">
                        @php $am = old('accounting_method','accrual'); @endphp
                        <option value="accrual" {{ $am==='accrual'?'selected':'' }}>Accrual basis</option>
                        <option value="cash" {{ $am==='cash'?'selected':'' }}>Cash basis</option>
                    </select>
                </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-12 gap-3 items-end max-w-xl">
                            <div class="sm:col-span-4">
                                ...
                                <div class="relative mt-1">
                                    <input id="from" type="date" name="from" value="{{ old('from', date('Y-m-d')) }}" class="w-full pr-10 border-gray-300 rounded-md shadow-sm focus:ring-2 focus:ring-blue-200 focus:border-blue-500" />
                            ...
                            <div class="sm:col-span-4">
                                ...
                                <div class="relative mt-1">
                                    <input id="to" type="date" name="to" value="{{ old('to', date('Y-m-d')) }}" class="w-full pr-10 border-gray-300 rounded-md shadow-sm focus:ring-2 focus:ring-blue-200 focus:border-blue-500" />
                                    <i class="far fa-calendar-alt absolute right-3 top-3.5 text-gray-400"></i>
                                </div>
                            </div>
                            <div class="sm:col-span-4">
                                <label for="column_label" class="block text-sm font-medium text-gray-700">Column name</label>
                                <input id="column_label" type="text" name="column_label" placeholder="Optional" value="{{ old('column_label') }}" class="mt-1 w-full border-gray-300 rounded-md shadow-sm focus:ring-2 focus:ring-blue-200 focus:border-blue-500" />
                            </div>
                        </div>
                    </div>
